#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Object-oriented training script for latent diffusion UNet stage with
validation using EMA, best-MSE and best-FID checkpointing on the same interval,
FID evaluation, gradient checkpointing, and nested dropout.
"""
...
class LDMTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        # Compute (or load) latent stats after Accelerator.prepare
    ...
    def _load_config(self):
        ...
        self.dataset_name      = self.ds_cfg.get("name", "").upper()
        self.is_class_cond     = self.dataset_name == "IMAGENET"
    ...
    def _prepare_class_labels(self, labels, *, drop_labels: bool = False):
        if not self.is_class_cond:
            return None
        ...
        for t in noise_scheduler.timesteps:
            with torch.no_grad():
                if self.is_class_cond:
                    ...
                    noise_pred = unet(sample=latents, timestep=t).sample
        ...
        while self.global_step < self.max_steps:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                if self.tr_cfg.get("nd"):
                    ...
                    loss_normal = F.mse_loss(pred, noise, reduction="mean")
                    loss_nd = F.mse_loss(pred_nd, noise, reduction="mean")
                    loss = (1 - self.alpha) * loss_normal + self.alpha * loss_nd
                else:
                    pred = self._model_predict(noisy, t, class_labels)
    ...
    def _validate_and_log(self, step: int):
        ...
        with torch.no_grad():
            for vimgs, vlabels in self.val_loader:
                ...
                val_class_labels = self._prepare_class_labels(vlabels, drop_labels=False)
        ...
        if step % self.fid_steps == 0:
            ...
            with torch.no_grad():
                for real_imgs, real_labels in self.val_loader:
                    ...
                    real_class_labels = self._prepare_class_labels(real_labels, drop_labels=False)
                    bs = real_imgs.size(0)
                    ...
                    fake_01 = self.generate_images(
                        ...
                        bs, self.device, class_labels=real_class_labels)
            ...
            # checkpoint dict with full state for potential resuming
            ckpt = {
                "step":        step,
                "global_step": self.global_step,
                "model_state": self.accelerator.unwrap_model(self.model).state_dict(),
    ...
    def _log_samples(self, step: int):
        ...
        with torch.no_grad():
            sample_labels = None
            if self.is_class_cond:
                sample_labels = torch.arange(32, device=self.device, dtype=torch.long) % self.num_classes
            samples = self.generate_images(self.z_dummy,
                self.vae, self.model,
                self.noise_sched_sample,
                32, self.device, class_labels=sample_labels
            )
        self.ema_model.restore(self.model.parameters())
